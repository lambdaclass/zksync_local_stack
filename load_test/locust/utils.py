...
def erc20_transfer_command(host, from_pk, to_address, amount, contract_address):
    a = f"zksync-era-cli --host {host} --l2-port 3050 send --contract {contract_address} --function 'transfer(address, uint256)' --args {to_address} {amount} --private-key {from_pk} --chain-id 270"
    print(a)
    return a